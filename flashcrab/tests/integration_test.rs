...
mod tests {
    use {
        ...
        embassy_stm32::{
            ...
            mode::Blocking,
            ospi::{
                AddressSize, Config as OspiConfig, DummyCycles, Instance, MemorySize, MemoryType,
                Ospi, OspiWidth, TransferConfig,
            },
            peripherals::{self, HASH, USB_OTG_HS},
            time::Hertz,
            uid::uid_hex,
            usb::InterruptHandler as USBInterruptHandler,
            Config as BoardConfig, Peripherals,
        },
        embassy_time::Timer,
        defmt_rtt as _,
    };
    ...
    #[test]
    async fn test_dac_setup(p: Peripherals) {
        ...
        info!("DAC setup completed successfully");
        core::assert!(true);
    }

    #[test]
    async fn test_hardware_hasher(p: Peripherals) {
        ...
        let all_zeros = hw_digest.iter().all(|&x| x == 0);
        core::assert!(!all_zeros, "Hash should not be all zeros");
    }

    #[test]
    async fn test_ospi_flash_operations(p: Peripherals) {
        ...
        // Flash ID should not be all zeros or all 0xFF
        let all_zeros = flash_id.iter().all(|&x| x == 0);
        let all_ones = flash_id.iter().all(|&x| x == 0xFF);
        core::assert!(!all_zeros && !all_ones, "Flash ID should be valid");
        ...
        // Verify write/read worked correctly
        core::assert_eq!(wr_buf, rd_buf, "Written and read data should match");
    }

    #[test]
    async fn test_led_sequence(p: Peripherals) {
        ...
        // Test LED sequence for a short duration
        for _ in 0..3 {
            led_1.set_high();
        ...
        info!("LED sequence test completed");
        core::assert!(true);
    }

    // Include the FlashMemory implementation
